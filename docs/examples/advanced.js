...
async function initializeGraph() {
    ...
    function getCurrentTitle() {
        return isModernEra
            ? 'European Diplomatic Network 2024'
            : 'European Diplomatic Network 1914';
    }
    ...
    async function updateEra() {
        ...
        console.log(
            `✅ Successfully switched to ${isModernEra ? '2024' : '1914'} diplomatic network`
        );
    }
    ...
    window.graph.on('filtered', data => {
        console.log(
            `🔍 Graph filtered by ${data.nodeId}, showing ${data.visibleNodes.length} nodes`
        );
    });
