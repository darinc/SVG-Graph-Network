...
function initializeWhenReady() {
    ...
    if (typeof GraphNetwork !== 'undefined') {
        console.log('GraphNetwork found, initializing graph...');
        initializeGraph();
    } else {
...
function initializeGraph() {
    ...
    window.addRandomNode = function () {
        ...
        for (let i = 0; i < numConnections; i++) {
            ...
            const randomLabel =
                connectionLabels[Math.floor(Math.random() * connectionLabels.length)];
    ...
    window.deleteRandomNode = function () {
        ...
        const connectedEdges = currentData.links.filter(
            link => link.source === randomNode.id || link.target === randomNode.id
        );
