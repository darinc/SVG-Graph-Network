...
export class GraphNetwork {
    ...
    createSettingsPanel() {
        ...
        controls.forEach(control => {
            ...
            slider.addEventListener('input', e =>
                this.handleSliderChange(control.id, e.target.value)
            );
        ...
        nodesArray.forEach(node => {
            ...
            switch (node.data.shape) {
                case 'rectangle':
                    ...
                    hiddenIndicator = document.createElementNS(
    ...
    setupEventListeners() {
        ...
        this.nodeGroup.addEventListener(
            'touchstart',
            e => {
                e.preventDefault();
    ...
    handleSvgMouseMove(e) {
        if (this.isDragging && this.dragNode) {
            ...
            const newX =
                (e.clientX - rect.left - this.transformState.x) / this.transformState.scale;
    ...
    handleWheel(e) {
        ...
        const newScale =
            e.deltaY < 0
                ? oldScale * this.config.zoomSensitivity
                : oldScale / this.config.zoomSensitivity;

        this.transformState.scale = Math.max(0.2, Math.min(2.0, newScale));

        this.transformState.x =
            mouseX - (mouseX - this.transformState.x) * (this.transformState.scale / oldScale);
        this.transformState.y =
            mouseY - (mouseY - this.transformState.y) * (this.transformState.scale / oldScale);
        ...
        if (touches.length === 1) {
            ...
            if (this.isDragging && this.dragNode) {
                ...
                this.dragNode.position.y =
                    (touch.clientY - rect.top - this.transformState.y) / this.transformState.scale;
                this.showTooltip(this.dragNode);
            ...
            this.transformState.x =
                zoomCenterX -
                (zoomCenterX - this.transformState.x) * (this.transformState.scale / oldScale);
            this.transformState.y =
                zoomCenterY -
                (zoomCenterY - this.transformState.y) * (this.transformState.scale / oldScale);
    ...
    updateGraphView() {
        ...
        this.links.forEach(link => {
            const isVisible =
                !nodesToDisplay ||
                (nodesToDisplay.has(link.source.data.id) &&
                    nodesToDisplay.has(link.target.data.id));
    ...
    zoomIn() {
        ...
        this.transformState.x =
            centerX - (centerX - this.transformState.x) * (this.transformState.scale / oldScale);
        this.transformState.y =
            centerY - (centerY - this.transformState.y) * (this.transformState.scale / oldScale);
    ...
    updateForces() {
        ...
        for (let i = 0; i < nodesArray.length; i++) {
            for (let j = i + 1; j < nodesArray.length; j++) {
                ...
                if (
                    this.filteredNodes &&
                    (!this.filteredNodes.has(nodeA.data.id) ||
                        !this.filteredNodes.has(nodeB.data.id))
                ) {
                    continue;
                }
                ...
                const repulsionForce = diff
                    .normalize()
                    .multiply(this.config.repulsionStrength / (distance * distance));
                ...
                if (nodeA.data.type === nodeB.data.type && this.config.groupingStrength > 0) {
                    const attractionForce = diff
                        .normalize()
                        .multiply(-this.config.groupingStrength * distance);
        ...
        this.links.forEach(link => {
            if (
                this.filteredNodes &&
                (!this.filteredNodes.has(link.source.data.id) ||
                    !this.filteredNodes.has(link.target.data.id))
            ) {
                return;
            }
            ...
            const attractionForce = diff
                .normalize()
                .multiply(this.config.attractionStrength * distance);
    ...
    animate() {
        if (!this.isPanning) {
            // Allow physics during drag, but not during pan
            this.updateForces();
    ...
    removeNode(nodeId) {
        ...
        this.links = this.links.filter(
            link => link.source.data.id !== nodeId && link.target.data.id !== nodeId
        );
